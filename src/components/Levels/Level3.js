...
const words = ['CODE','ROAR', 'CLAY', 'NOTE', 'WANT'];
...
const Level3 = () => {
  ...
  const handleGoBack = () => {
    // 뒤로가기 버튼을 클릭했을 때 수행할 동작을 여기에 추가
        ...
        {!allWordsDisplayed && currentWord && (
          ...
          <div style={{ marginTop: '10px', textAlign: 'center' }}>
            {currentWord.split('').map((letter, index) => (
              <span key={index} style={{ fontSize: index === currentLetterIndex ? '5em' : '3em', color: index === currentLetterIndex ? 'purple' : 'black', padding: '0 5px' }}>
                {letter}
              </span>
            ))}
          </div>
